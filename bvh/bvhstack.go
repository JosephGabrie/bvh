...
func (s *orthStack) intersectsNext(orth *Orthotope, delta *Coordinate) (*BVol, float32) {
	bvol, index := s.peek()
	distance := float32(-1)
	for bvol.depth > 0 {
		if index >= 2 {
...
func (s *orthStack) Intersects(orth *Orthotope, delta *Coordinate) (*Orthotope, float32) {
	if !s.HasNext() {
		return nil, -1
	}
...
func (s *orthStack) Score() float32 {
	s.Reset()
	var score float32
